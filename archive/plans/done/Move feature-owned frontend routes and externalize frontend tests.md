...
- Main gaps: page ownership is not explicit by feature, route state is not URL-based, `ui-shell` is historical and vague, the current frontend test layout does not match the chosen rule to keep tests outside `src/`, the live library still drifts from `docs/ui/video-library.png`, and the live review route still needs a mockup-aligned loaded shell plus a designed failure state instead of broken bootstrap presentation.
