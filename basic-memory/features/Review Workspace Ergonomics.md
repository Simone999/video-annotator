...
## Current State
- Shipped behavior: app host now uses real browser routes, with `/` opening backend-backed library shell chrome and `/review/:videoId` resolving through a temporary app-level adapter into `frontend/src/app/live-review-app.tsx`. Default library `Open Review` handoff is still local app state until later route-ownership tasks move library and review pages into their feature folders. Live review still keeps playback visible on one stage, overlays the canonical exact frame only while paused, auto-loads the first annotated frame when manifest data has one, falls back to frame `0` otherwise, and keeps manual-box plus SAM2 controls reachable from the same workspace. Operators can now jump directly through manifest-backed annotated frames and keyframes, use paused-only keyboard shortcuts for routine movement, and adjust selected-mask overlay opacity locally without changing persisted data.
- Verification state: lower fixture review-shell proof still lives in `frontend/src/features/ui-shell/shell-host.test.tsx`, while `frontend/src/app/App.test.tsx` now proves the live library host, ignored legacy query switching, `/review/:videoId`, and `*` through mocked HTTP. `frontend/src/app/live-review-app.test.tsx` now proves single-stage layout, paused-only mutation gating, useful landing, annotated/keyframe jumps, keyboard shortcuts, and mask-opacity controls. Browser smoke on the real local stack saved `/tmp/us-014-live-library-shell.png`, `/tmp/us-014-live-review-entry.png`, `/tmp/us-015-live-single-stage-review.png`, `/tmp/us-016-review-navigation-controls.png`, `/home/simone/.dev-browser/tmp/us001-library-route.png`, `/home/simone/.dev-browser/tmp/us001-review-route.png`, and `/home/simone/.dev-browser/tmp/us001-missing-route.png`.
...
| MAN-002 | Live backend-backed library opens review and returns to library | Run `npm run backend:dev:e2e` and `npm run frontend:dev:e2e`, open `http://127.0.0.1:5174` | Wait for backend library cards, open one video with `Open Review`, then use `Back to Library` | Backend summary-backed library opens review on the single-stage live surface and local back navigation returns to library | ✅ Done | Browser smoke on 2026-04-21 saved `/tmp/us-014-live-library-shell.png` and `/tmp/us-014-live-review-entry.png` |
...
- [routing] App host now uses real browser routes `/`, `/review/:videoId`, and `*`; temporary review adapter still lives in `frontend/src/app/router.tsx` until feature-owned page extraction lands #frontend #routing
