...
- Verification state: `frontend/tests/unit/frontend-structure/ui-shell-cleanup.test.ts` freezes removal of `frontend/src/features/ui-shell/` plus stale `ui-shell` runtime naming in `frontend/src/`, `frontend/tests/unit/frontend-structure/frontend-test-tree.test.ts` freezes the required `frontend/tests/{unit,integration}` split and bans frontend Vitest files under `frontend/src/`, `frontend/tests/integration/video-library/video-library-screen.test.tsx` proves route-owned library chrome keeps accessible controls without raw icon fallback text while still freezing propagation-progress gating, `frontend/tests/integration/app/app-routes.test.tsx` proves routed app-host behavior for `/`, `/review/:videoId`, and `*` plus operator-facing library folder copy and backend frame-preview URLs, `frontend/tests/integration/video-review/review-page.test.tsx` proves route-param handoff plus back navigation into feature-owned live review screen, and `frontend/tests/integration/video-review/live-review-screen.test.tsx` proves single-stage layout, direct-load bootstrap from `initialVideoId`, paused-only mutation gating, useful landing, annotated/keyframe jumps, keyboard shortcuts, and mask-opacity controls. Committed browser proof still lives in `tests/e2e/specs/routes.spec.ts` for route flow and `tests/e2e/specs/review-navigation.spec.ts` for seeded manifest-jump controls, while fresh local browser smoke on 2026-04-22 confirmed `/` -> `Open Review bedroom.mp4` -> `Back to Library` on clean port `3100`, and focused Playwright smoke on the same clean port confirmed backend frame previews on `/` plus first navigation into `/review/video-2d62649f3590f8d0`, saving `/tmp/us005-route-cleanup-smoke.png`, `/tmp/us008-library-route.png`, and `/tmp/us008-library-to-review.png`. Historical browser smoke on the real local stack saved `/tmp/us-014-live-library-shell.png`, `/tmp/us-014-live-review-entry.png`, `/tmp/us-015-live-single-stage-review.png`, `/tmp/us-016-review-navigation-controls.png`, `/home/simone/.dev-browser/tmp/us001-library-route.png`, `/home/simone/.dev-browser/tmp/us001-review-route.png`, `/home/simone/.dev-browser/tmp/us001-missing-route.png`, `/home/simone/.dev-browser/tmp/us002-library-route.png`, `/home/simone/.dev-browser/tmp/us002-review-route.png`, `/home/simone/.dev-browser/tmp/us003-review-route-refresh.png`, and `/home/simone/.dev-browser/tmp/us004-review-route-feature-owned.png`.
- Current blockers: live review still does not render timeline or selected-range controls, and it still does not call the selected-object summary route, so inspector `frames / propagated / corrected / mask_confidence` truth is absent from shipped UI. Stale long-lived backend listeners on `127.0.0.1:8000` can still fake `/manifest` failures during direct-route smoke, so browser proof should restart fresh current-code backend before judging route regressions. Fresh local rerun of `tests/e2e/specs/routes.spec.ts` on clean port `3100` still failed at the seeded review-route refresh step, timing out while waiting for `Canonical frame 0` after direct reload, so route-refresh proof remains unstable and should be revisited by later routing-verification work. Backend summary route still keeps `mask_confidence` and `track_summary.corrected` as `null` because current persistence does not store SAM2 confidence or reviewer-correction provenance yet. Export state is still unshipped runtime truth, so backend summary derivation will not emit `exported` until real export persistence exists.
...
- Fresh rerun of committed `tests/e2e/specs/routes.spec.ts` failed once on 2026-04-22 because Playwright reused a stale backend already bound to `127.0.0.1:8000`; route showed designed `Review unavailable` plus real `Failed to fetch` text. Treat that as browser-environment drift, not proof that fresh seeded route bootstrap is broken.
...
- [testing] Fresh local rerun of `tests/e2e/specs/routes.spec.ts` still failed at seeded review-route refresh while waiting for `Canonical frame 0`, so later routing-proof work must treat direct reload as unstable until that browser path is re-verified #testing #frontend #routing
...
- [testing] `frontend/tests/integration/video-review/live-review-screen.test.tsx` now proves the single-stage live surface, split-pane removal, and paused-only mutation gating, while `tests/e2e/specs/review-navigation.spec.ts` adds real-browser seeded manifest-jump proof on top of historical smoke `/tmp/us-015-live-single-stage-review.png` #testing #frontend #backend
