...
## Current State
- Shipped behavior: app host now uses real browser routes, with `/` owned by `frontend/src/features/video-library/pages/library-page.tsx` and `/review/:videoId` still resolving through a temporary app-level adapter into `frontend/src/app/live-review-app.tsx`. Default library `Open Review` now uses real router navigation to `/review/:videoId`, while fixture-shell coverage reuses the moved `video-library` screen instead of keeping a second `ui-shell` library page copy. Live review still keeps playback visible on one stage, overlays the canonical exact frame only while paused, auto-loads the first annotated frame when manifest data has one, falls back to frame `0` otherwise, and keeps manual-box plus SAM2 controls reachable from the same workspace. Operators can now jump directly through manifest-backed annotated frames and keyframes, use paused-only keyboard shortcuts for routine movement, and adjust selected-mask overlay opacity locally without changing persisted data.
- Verification state: lower fixture review-shell proof still lives in `frontend/src/features/ui-shell/shell-host.test.tsx`, while `frontend/src/app/App.test.tsx` now proves the routed live library host, ignored legacy query switching, `/review/:videoId`, and `*` through mocked HTTP. `frontend/src/features/video-library/library-page.test.tsx` keeps the moved feature's propagation-progress card rule under `video-library`, and `frontend/src/app/live-review-app.test.tsx` still proves single-stage layout, paused-only mutation gating, useful landing, annotated/keyframe jumps, keyboard shortcuts, and mask-opacity controls. Browser smoke on the real local stack saved `/tmp/us-014-live-library-shell.png`, `/tmp/us-014-live-review-entry.png`, `/tmp/us-015-live-single-stage-review.png`, `/tmp/us-016-review-navigation-controls.png`, `/home/simone/.dev-browser/tmp/us001-library-route.png`, `/home/simone/.dev-browser/tmp/us001-review-route.png`, `/home/simone/.dev-browser/tmp/us001-missing-route.png`, `/home/simone/.dev-browser/tmp/us002-library-route.png`, and `/home/simone/.dev-browser/tmp/us002-review-route.png`.
- Current blockers: live review still does not render timeline or selected-range controls, and it still does not call the selected-object summary route, so inspector `frames / propagated / corrected / mask_confidence` truth is absent from shipped UI. Current local browser smoke for direct `/review/:videoId` also shows backend `/api/videos/{video_id}/manifest` returning `500`, so route resolution works but full selected-video bootstrap can still fail until that backend bug is fixed. Backend summary route still keeps `mask_confidence` and `track_summary.corrected` as `null` because current persistence does not store SAM2 confidence or reviewer-correction provenance yet. Export state is still unshipped runtime truth, so backend summary derivation will not emit `exported` until real export persistence exists.
...
| INT-005 | frontend | Live review uses one single-stage surface, lands on first useful frame, and jumps through annotated or keyframe markers | Proves shipped review navigation ergonomics without treating playback position as annotation truth | `LiveReviewApp` with `MSW` request-boundary stubs | automated | `frontend/src/app/live-review-app.test.tsx` |
| INT-006 | frontend | Live review keyboard shortcuts, local mask opacity, and paused-only mutation gating stay coherent on one stage | Proves operator-speed controls and review-stage guardrails now visible in shipped workspace | `LiveReviewApp` with `MSW` request-boundary stubs | automated | `frontend/src/app/live-review-app.test.tsx` |
| INT-007 | frontend | Live review loads selected-object summary from backend route and shows `frames / propagated / corrected / mask_confidence` truth in inspector | Future UI must prove inspector numbers come from backend summary route instead of ad-hoc frontend guesses | `LiveReviewApp` with `MSW` or real request boundary once summary wiring lands | blocked until UI wiring exists | missing selected-object summary fetch and rendering in `frontend/src/app/live-review-app.tsx`; tracked by `[[Wire live selected-object summary]]` |
| INT-008 | frontend | Live review renders timeline and selected-range controls, then keeps range-dependent inspector and propagation flows coherent | Future UI must prove selected-range ergonomics before PRD counter semantics can be exercised end-to-end | `LiveReviewApp` with `MSW` or local stack once controls exist | blocked until controls exist | missing timeline and selected-range controls; tracked by `[[Add live review timeline and selected range controls]]` |
...
| MAN-004 | App host routes `/`, `/review/:videoId`, and `*` resolve through real browser route map | Use current local frontend dev stack on `http://127.0.0.1:5174` with backend on `127.0.0.1:8000` | Open `/`, direct-load `/review/video-2d62649f3590f8d0`, then open `/missing-route` and use `Back to Library` | Library route loads, review route resolves and applies URL video selection, and unknown route returns cleanly to `/` | ⚠️ Partially | `dev-browser` smoke on 2026-04-21 saved `/home/simone/.dev-browser/tmp/us001-library-route.png`, `/home/simone/.dev-browser/tmp/us001-review-route.png`, and `/home/simone/.dev-browser/tmp/us001-missing-route.png`; direct review route resolved and selected `bedroom.mp4`, but current backend `/manifest` returned `500`, so full workspace bootstrap stayed blocked |

## Observations
- [status] Ergonomics target changed from separate panes to single-stage review surface #frontend #ux
- [status] Default frontend entry now boots backend-backed library route from `frontend/src/features/video-library/pages/library-page.tsx`, and review still hands off into `frontend/src/app/live-review-app.tsx` through a temporary adapter #frontend #ui
...
- [routing] App host now uses real browser routes `/`, `/review/:videoId`, and `*`; `/` is feature-owned under `frontend/src/features/video-library/pages/library-page.tsx`, while temporary review adapter still lives in `frontend/src/app/router.tsx` until feature-owned review extraction lands #frontend #routing
...
- [testing] `frontend/src/app/App.test.tsx` now proves live library loading, empty or error states, and real `/review/:videoId` handoff through the default app host with mocked HTTP, while `frontend/src/features/video-library/library-page.test.tsx` keeps moved card-state proof local to the owning feature and `frontend/src/features/ui-shell/shell-host.test.tsx` keeps lower fixture-shell seam coverage by mocking the loader #testing #frontend #ui
...
- [gotcha] Current local backend `/api/videos/{video_id}/manifest` can return `500` during direct `/review/:videoId` smoke, so route resolution and selected-button state may verify correctly even when full review bootstrap is blocked #frontend #backend #routing
