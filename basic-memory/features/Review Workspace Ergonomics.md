...
| INT-003 | frontend | Default host renders backend-backed library cards, empty or error states, and `Open Review` or `Back to Library` handoff | Proves shipped library chrome and review entry on real app host with fake HTTP only at request boundary | `App` with mocked `/api/videos` and mocked live review host boundary | automated | `frontend/tests/component/app/app-routes.test.tsx` |
...
| INT-005 | frontend | Live review uses one single-stage surface, lands on first useful frame, and jumps through annotated or keyframe markers | Proves shipped review navigation ergonomics without treating playback position as annotation truth | `LiveReviewScreen` with `MSW` request-boundary stubs | automated | `frontend/tests/component/video-review/live-review-screen.test.tsx` |
| INT-006 | frontend | Live review keyboard shortcuts, local mask opacity, and paused-only mutation gating stay coherent on one stage | Proves operator-speed controls and review-stage guardrails now visible in shipped workspace | `LiveReviewScreen` with `MSW` request-boundary stubs | automated | `frontend/tests/component/video-review/live-review-screen.test.tsx` |
...
- [testing] `frontend/tests/unit/frontend-structure/ui-shell-cleanup.test.ts` now freezes deletion of `frontend/src/features/ui-shell/`, while `frontend/tests/component/video-review/review-page.test.tsx` covers route-param handoff plus route-owned back navigation, `frontend/tests/component/app/app-routes.test.tsx` proves live library loading, empty or error states, and real `/review/:videoId` host routing, and `frontend/tests/component/video-library/video-library-screen.test.tsx` keeps moved card-state proof local to the owning feature #testing #frontend #ui
...
- [testing] `frontend/tests/component/video-review/live-review-screen.test.tsx` now proves the single-stage live surface, split-pane removal, and paused-only mutation gating, while `tests/e2e/specs/review-navigation.spec.ts` adds real-browser seeded manifest-jump proof on top of historical smoke `/tmp/us-015-live-single-stage-review.png` #testing #frontend #backend
