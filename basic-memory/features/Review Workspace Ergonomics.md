...
## Current State
- Shipped behavior: app host now uses real browser routes, with `/` owned by `frontend/src/features/video-library/pages/library-page.tsx` and `/review/:videoId` owned by `frontend/src/features/video-review/pages/review-page.tsx`. That feature page reads `videoId` from route params and renders feature-owned live review screen `frontend/src/features/video-review/components/live-review-screen.tsx`. Historical `ui-shell` runtime code and fixture-only review route behavior are deleted, so shipped frontend ownership now stays inside explicit `video-library` and `video-review` feature paths only. Live review still keeps playback visible on one stage, overlays the canonical exact frame only while paused, auto-loads the first annotated frame when manifest data has one, falls back to frame `0` otherwise, and keeps manual-box plus SAM2 controls reachable from the same workspace. Operators can now jump directly through manifest-backed annotated frames and keyframes, use paused-only keyboard shortcuts for routine movement, and adjust selected-mask overlay opacity locally without changing persisted data.
- Verification state: `frontend/tests/unit/frontend-structure/ui-shell-cleanup.test.ts` freezes removal of `frontend/src/features/ui-shell/` plus stale `ui-shell` runtime naming in `frontend/src/`, `frontend/tests/unit/frontend-structure/frontend-test-tree.test.ts` freezes the required `frontend/tests/{unit,integration,e2e}` homes and bans frontend Vitest files under `frontend/src/`, `frontend/tests/unit/frontend-structure/playwright-test-tree.test.ts` freezes the moved Playwright spec and fixture tree under `frontend/tests/e2e/`, `frontend/tests/integration/video-library/video-library-screen.test.tsx` proves route-owned library chrome keeps accessible controls without raw icon fallback text while still freezing propagation-progress gating, `frontend/tests/integration/app/app-routes.test.tsx` proves routed app-host behavior for `/`, `/review/:videoId`, and `*` plus operator-facing library folder copy and backend frame-preview URLs, `frontend/tests/integration/video-review/review-page.test.tsx` proves route-param handoff plus back navigation into feature-owned live review screen, and `frontend/tests/integration/video-review/live-review-screen.test.tsx` proves single-stage layout, direct-load bootstrap from `initialVideoId`, paused-only mutation gating, useful landing, annotated/keyframe jumps, keyboard shortcuts, and mask-opacity controls. Committed browser proof now lives in `frontend/tests/e2e/routes.spec.ts` for route flow and `frontend/tests/e2e/review-navigation.spec.ts` for seeded manifest-jump controls, while fresh local browser smoke on 2026-04-22 confirmed `/` -> `Open Review bedroom.mp4` -> `Back to Library` on clean port `3100`, focused Playwright smoke on the same clean port confirmed backend frame previews on `/` plus first navigation into `/review/video-2d62649f3590f8d0`, and fresh committed Playwright rerun on the same clean port passed route refresh plus seeded manifest jumps, saving `/tmp/us005-route-cleanup-smoke.png`, `/tmp/us008-library-route.png`, and `/tmp/us008-library-to-review.png`. Historical browser smoke on the real local stack saved `/tmp/us-014-live-library-shell.png`, `/tmp/us-014-live-review-entry.png`, `/tmp/us-015-live-single-stage-review.png`, `/tmp/us-016-review-navigation-controls.png`, `/home/simone/.dev-browser/tmp/us001-library-route.png`, `/home/simone/.dev-browser/tmp/us001-review-route.png`, `/home/simone/.dev-browser/tmp/us001-missing-route.png`, `/home/simone/.dev-browser/tmp/us002-library-route.png`, `/home/simone/.dev-browser/tmp/us002-review-route.png`, `/home/simone/.dev-browser/tmp/us003-review-route-refresh.png`, and `/home/simone/.dev-browser/tmp/us004-review-route-feature-owned.png`.
- Current blockers: live review still does not render timeline or selected-range controls, and it still does not call the selected-object summary route, so inspector `frames / propagated / corrected / mask_confidence` truth is absent from shipped UI. Stale long-lived backend listeners on `127.0.0.1:8000` can still fake `/manifest` failures during direct-route smoke, so browser proof should restart fresh current-code backend before judging route regressions. Fresh local rerun of `frontend/tests/e2e/routes.spec.ts` on clean port `3100` passed route refresh after Playwright moved its frontend spec tree under `frontend/tests/e2e/`, so route-refresh proof is now committed browser evidence instead of a deferred blocker. Backend summary route still keeps `mask_confidence` and `track_summary.corrected` as `null` because current persistence does not store SAM2 confidence or reviewer-correction provenance yet. Export state is still unshipped runtime truth, so backend summary derivation will not emit `exported` until real export persistence exists.
...
| INT-003 | frontend | Default host renders backend-backed library cards, short local-source copy, backend frame-preview URLs, empty or error states, and `Open Review` or `Back to Library` handoff | Proves shipped library chrome and review entry on real app host with fake HTTP only at request boundary | `App` with mocked `/api/videos` and mocked live review host boundary | automated | `frontend/tests/integration/app/app-routes.test.tsx` |
| INT-004 | frontend structure | `frontend/src` stays free of `ui-shell` runtime folders, imports, and stale runtime naming after route ownership cleanup | Prevents historical shell runtime from silently re-entering shipped route-owned frontend paths | Node-environment filesystem scan over `frontend/src` | automated | `frontend/tests/unit/frontend-structure/ui-shell-cleanup.test.ts` |
| INT-005 | frontend | Live review uses one single-stage surface, lands on first useful frame, and jumps through annotated or keyframe markers | Proves shipped review navigation ergonomics without treating playback position as annotation truth | `LiveReviewScreen` with `MSW` request-boundary stubs | automated | `frontend/tests/integration/video-review/live-review-screen.test.tsx` |
| INT-006 | frontend | Live review keyboard shortcuts, local mask opacity, and paused-only mutation gating stay coherent on one stage | Proves operator-speed controls and review-stage guardrails now visible in shipped workspace | `LiveReviewScreen` with `MSW` request-boundary stubs | automated | `frontend/tests/integration/video-review/live-review-screen.test.tsx` |
...
| --- | --- | --- | --- | --- | --- |
| E2E-001 | Open `/`, inspect route-owned live library chrome, navigate into `/review/:videoId`, refresh, and return to `/` in a real browser | Freezes the default reviewer-visible route journey after feature-owned pages land without relying on fixture shell proof | `tests/e2e/global.setup.ts`, real frontend, real FastAPI app, explicit baseline seed | automated | `frontend/tests/e2e/routes.spec.ts` |
| E2E-002 | Seed manifest markers, direct-load `/review/:videoId`, and jump through annotated frames plus keyframes in a real browser | Freezes reviewer navigation ergonomics that depend on persisted manifest markers rather than playback time | `tests/e2e/global.setup.ts`, `frontend/tests/e2e/fixtures/review-navigation.ts`, real frontend, real FastAPI app, explicit scenario seed | automated | `frontend/tests/e2e/review-navigation.spec.ts` |
...
| --- | --- | --- | --- | --- | --- | --- |
| MAN-001 | Route-owned library opens review and returns cleanly | Run frontend dev stack on `http://127.0.0.1:3100` with seeded backend on `127.0.0.1:8000` | Open `/`, click `Open Review bedroom.mp4`, wait for review surface, then use `Back to Library` | Route-owned library opens live review and returns to `/` without reviving deleted shell runtime seams | ✅ Done | Playwright smoke on 2026-04-22 saved `/tmp/us005-route-cleanup-smoke.png` |
...
| MAN-003 | Live single-stage review shows useful landing plus navigation controls | Run fresh current-code backend and frontend dev stack, open one real `/review/:videoId` route | Open one indexed video, confirm single-stage layout, jump through annotated or keyframe markers, use keyboard shortcuts, and adjust mask-opacity slider | Review stays on one stage, useful landing works, navigation shortcuts follow canonical frame state, and mask-opacity changes stay local | ❌ Not Done | Routing changed |
...
| MAN-005 | Route-owned live library shows backend frame previews and short folder copy | Use clean frontend dev stack on `http://127.0.0.1:3100` with seeded backend on `127.0.0.1:8000` | Open `/`, wait for backend library cards, confirm preview image request comes from backend frame route and raw source path text is absent, then click `Open Review bedroom.mp4` | Default library route shows real backend frame previews, short operator-facing local-folder copy, and still navigates into `/review/:videoId` | ✅ Done | Focused Playwright smoke on 2026-04-22 saved `/tmp/us008-library-route.png` and `/tmp/us008-library-to-review.png`; first preview used `http://127.0.0.1:8000/api/videos/video-2d62649f3590f8d0/frame/0` and navigation landed on `/review/video-2d62649f3590f8d0` |

## Recent Verification

- `US-009` added route-owned loading and unavailable-shell proof in `frontend/tests/integration/video-review/live-review-screen.test.tsx`, so direct `/review/:videoId` routes now fail visibly without chooser-first UI or fake loaded data.
- Fresh browser smoke on 2026-04-22 used `npm run backend:bootstrap:e2e`, fresh `npm run backend:dev:e2e` on `127.0.0.1:8000`, and `FRONTEND_E2E_PORT=3100 npm run frontend:dev:e2e`, then opened `/review/video-2d62649f3590f8d0`, refreshed same route, and returned with `Back to Library`; screenshots: `/tmp/us009-review-route-open.png` and `/tmp/us009-review-route-refresh.png`.
- Fresh rerun of committed `frontend/tests/e2e/routes.spec.ts` plus `frontend/tests/e2e/review-navigation.spec.ts` passed on 2026-04-22 with clean port `3100` and explicit baseline seed after Playwright pointed Chromium at `frontend/tests/e2e/`; keep treating stale reused backends on `127.0.0.1:8000` as browser-environment drift, not route regressions.
...
- [guardrail] Library UI must gate propagation progress on `state: in_progress`; fixture percent presence alone is not enough #progress #library #fixtures
...
- [testing] Route-owned library -> review -> back navigation now has current manual browser smoke evidence on clean port `3100`, while route-host and live-review integration suites keep lower automated proof #testing #frontend #browser
- [testing] `frontend/tests/unit/frontend-structure/ui-shell-cleanup.test.ts`, `frontend/tests/unit/frontend-structure/frontend-test-tree.test.ts`, and `frontend/tests/unit/frontend-structure/playwright-test-tree.test.ts` now freeze deleted legacy frontend structure plus the required `frontend/tests/{unit,integration,e2e}` homes, while `frontend/tests/integration/video-review/review-page.test.tsx` covers route-param handoff plus route-owned back navigation, `frontend/tests/integration/app/app-routes.test.tsx` proves live library loading, empty or error states, and real `/review/:videoId` host routing, and `frontend/tests/integration/video-library/video-library-screen.test.tsx` keeps moved card-state proof local to the owning feature #testing #frontend #ui
- [library] Route-owned library cards now use backend exact-frame previews from `/api/videos/{video_id}/frame/{last_reviewed_frame_idx ?? 0}` and short local-folder copy instead of generated placeholder art or raw `source_path` text #library #frontend #preview
- [testing] Fresh local rerun of `frontend/tests/e2e/routes.spec.ts` passed direct review-route refresh on clean port `3100` after Playwright moved frontend browser specs into `frontend/tests/e2e`, so direct reload is now committed browser proof instead of deferred route work #testing #frontend #routing
- [cleanup] Historical `ui-shell` runtime code, fixture review page, and shell-host integration seam are deleted; route-owned `video-library` and `video-review` paths are now the only shipped frontend runtime owners #frontend #cleanup #routing
- [testing] `frontend/tests/integration/video-review/live-review-screen.test.tsx` now proves the single-stage live surface, split-pane removal, and paused-only mutation gating, while `frontend/tests/e2e/review-navigation.spec.ts` adds real-browser seeded manifest-jump proof on top of historical smoke `/tmp/us-015-live-single-stage-review.png` #testing #frontend #backend
