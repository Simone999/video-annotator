...
## Current State
- Shipped behavior: session lifecycle, prompt-box shell, propagation jobs, polling, cancel, and reopen shell exist, and fake-adapter shell trust now has backend API plus live-review frontend integration coverage. Backend persistence now carries nullable `mask_confidence` on untouched `source = "sam2"` rows, clears it on manual rewrites, exposes it on immediate prompt responses and frame reads, and reuses it in selected-object summary reads without inventing values for manual or corrected rows. Live review inspector now renders backend-backed selected-object bbox, confidence, and frame or propagated or corrected counters from selected-object summary truth, while keeping null confidence or corrected values honest as `Unavailable`. Selected-object summary fetch lifecycle ships in frontend controller state with typed API parsing, current object or frame or propagation-range reloads, sync-keyed range derivation, and request-key gating so stale ready summaries do not paint or refetch with old range scopes during selection or frame changes. Shared transport footer now renders manifest markers plus selected-range controls with reviewer-facing range wording, while inspector keeps propagation actions and summary truth plus a visible selected-range label. Default `Sam2Service.prompt_box()` now lazily loads real runtime state on first prompt from `SAM2_CONFIG_PATH` plus `SAM2_CHECKPOINT_PATH`, keeps `POST /sam2/session` lightweight, persists real PNG masks, and returns explicit `503` failures when runtime config, dependencies, checkpoint, or requested device are unavailable. Default `Sam2Service.propagate()` now reuses that same lazy predictor/session state, maps `forward`, `backward`, and `both` requests onto official `predictor.propagate_in_video(...)` calls, encodes propagated PNG masks, recreates process-local runtime session state from the open DB row when backend memory lost it before propagation start, and keeps partial progress or cancel truth coherent through job status reads plus reopened frame annotations.
- Known gaps: honest real prompt or propagation browser proof still remains open in current shell because local SAM2 assets are not configured. Refine follow-up now routes through `[[Mask Editing and Cleanup]]`, not open m-3 runtime parity work.
- Current blockers: `SAM2_CONFIG_PATH` and `SAM2_CHECKPOINT_PATH` were not set in this shell on 2026-04-22, so full browser-visible runtime trust is still blocked on local environment setup.

## Verification Evidence
- Backend: `backend/tests/integration/api/test_sam2_shell_runtime.py` proves session create or reuse, immediate prompt response nullable confidence, prompt-box persistence, explicit prompt runtime-missing `503` failure, propagation job status reads, cancellation, close or reopen session flow, reopened persisted SAM2 masks, one real-service lazy prompt-loader path at real FastAPI boundary, one real-service propagation persistence path, one rehydrated real-service propagation start plus partial-progress cancel path after in-memory session loss, and one explicit failed-job path when propagation runtime setup is missing.
- Backend adapter boundary: `backend/tests/unit/services/test_sam2.py` now proves real propagation mapping for requested-object filtering, `both`-direction runtime-call ordering, and explicit missing-runtime setup errors.
- Frontend: `frontend/tests/integration/video-review/live-review-screen.test.tsx` proves live-review harness can run SAM2, show propagation queue or run or cancel states, and reopen persisted mask overlay with mocked HTTP boundary only. `frontend/tests/unit/video-review/use-sam2-workspace.test.tsx` proves session or prompt or propagation request state, success and failure dispatch, polling, and stale-response guards stay scoped to current selected video.
...
- Browser route proof: manual `backend:bootstrap:e2e` plus `backend:seed:e2e:review-navigation` plus fresh `backend:dev:e2e` or `frontend:dev:e2e`, then `FRONTEND_E2E_PORT=3100 npx playwright test -c tests/e2e/playwright.config.ts --project=chromium --no-deps frontend/tests/e2e/review-navigation.spec.ts` passed on 2026-04-22.
- Manual runtime: blocked. Current shell had no `SAM2_CONFIG_PATH` or `SAM2_CHECKPOINT_PATH`, so no honest real prompt or propagation browser proof could run in this task.
...
- [api] Prompt-box route now returns nullable `mask_confidence` in immediate same-frame response, not only after reload #sam2 #api #confidence
