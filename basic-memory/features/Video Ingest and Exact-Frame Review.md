...
| INT-004 | frontend | Live review opens one video, loads exact frame, and steps next or previous while playback stays contextual only | Proves reviewer-visible exact-frame workflow with real React state and fake HTTP only at request boundary | `LiveReviewScreen` with `MSW` stubs for detail, manifest, frame, and annotations routes | automated | `frontend/src/features/video-review/components/live-review-screen.test.tsx` |
...
- [testing] Frontend integration now proves route-param handoff in `frontend/src/features/video-review/pages/review-page.test.tsx` plus live review selection, exact-frame load, next-frame, and previous-frame flow in `frontend/src/features/video-review/components/live-review-screen.test.tsx` without treating playback source as canonical truth #testing #frontend #exact-frame
