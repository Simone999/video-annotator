...
## Current State
- Shipped behavior: explicit migration plus baseline indexing, list and detail reads, playback source, exact-frame fetch, jump, and step exist on core path. Default frontend startup now reaches the real indexed library through backend `/api/videos` after explicit bootstrap, and both library navigation plus direct `/review/:videoId` loads now enter feature-owned page `frontend/src/features/video-review/pages/review-page.tsx`, which renders single-stage live review surface `frontend/src/features/video-review/components/live-review-screen.tsx`. Route-owned library and review shells now follow the authoritative HTML chrome more closely with fixed top bar plus left rail layout, flatter summary strips, backend-backed selected-object inspector truth, and timeline-first transport chrome with manifest markers plus shared selected-range controls instead of fake runtime values. Automated proof lives in `backend/tests/integration/api/test_video_ingest_exact_frame.py`, `frontend/tests/integration/video-library/video-library-screen.test.tsx`, `frontend/tests/integration/video-review/review-page.test.tsx`, `frontend/tests/integration/video-review/live-review-screen.test.tsx`, and `frontend/tests/e2e/routes.spec.ts`.
- Known gaps: this note stays scoped to ingest and exact-frame truth. Direct timeline interaction wiring and range-driven propagation reuse still live in `[[SAM2 Shell and Runtime]]` and `[[m-2: Review Workspace PRD Parity]]`, while real `exported` truth still lives in `[[Export]]` plus `[[m-5: Export Workflow and Exported State]]`.
...
## Observations
- [status] Ingest and exact-frame foundations now ship on live library-first single-stage path with timeline-first transport chrome; later m-2 tasks now focus on interaction wiring instead of missing transport UI #review #video
- [truth] Playback may exist in UI, but backend `frame_idx` stays annotation truth #frames #frontend
- [scope] This note stays on ingest and exact-frame truth; route-owned shell parity plus library review-state truth up to timeline-first transport now ship here, while direct timeline interaction wiring still pairs with `[[SAM2 Shell and Runtime]]` #review #scope
...
- [testing] Frontend integration now proves route-param handoff in `frontend/tests/integration/video-review/review-page.test.tsx` plus live review selection, exact-frame load, timeline markers, selected-range footer, fallback frame jump, next-frame, and previous-frame flow in `frontend/tests/integration/video-review/live-review-screen.test.tsx` without treating playback source as canonical truth #testing #frontend #exact-frame
- [ui] Route-owned library and review shells now keep PRD-shaped chrome with backend-backed selected-object inspector truth, manifest-driven timeline markers, and exact-frame numeric fallback demoted behind secondary transport UI #frontend #ui #review
...
- [range] Live review now keeps one explicit inclusive selected-range controller state on canonical frame indices, and timeline-first transport now renders that shared range while click or drag wiring stays in follow-up m-2 tasks #review #range #exact-frame
