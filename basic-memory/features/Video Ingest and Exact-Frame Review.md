...
- Known gaps: this note stays scoped to ingest and exact-frame truth. Selected-object summary fetch now follows current object, canonical frame, and current propagation-range inputs, but inspector rendering plus explicit selected-range timeline controls still live in `[[SAM2 Shell and Runtime]]` and `[[m-2: Review Workspace PRD Parity]]`, while real `exported` truth still lives in `[[Export]]` plus `[[m-5: Export Workflow and Exported State]]`.
