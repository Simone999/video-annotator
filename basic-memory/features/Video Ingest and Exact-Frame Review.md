...
## Observations
- [status] Ingest and exact-frame foundations now ship on live library-first single-stage path; selected-range transport semantics still pair with `[[SAM2 Shell and Runtime]]` and current todo tasks #review #video
- [truth] Playback may exist in UI, but backend `frame_idx` stays annotation truth #frames #frontend
- [scope] This note stays on ingest and exact-frame truth; route-owned shell parity plus library review-state truth up to selected-object inspector summary now ship here, while selected-range controls still pair with `[[SAM2 Shell and Runtime]]` #review #scope
