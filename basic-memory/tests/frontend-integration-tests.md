...
- re-query `Exact frame canvas` after `Load frame`; exact-frame reload can remount the canvas node and stale refs can break later drag or resize steps
...
- [guardrail] Live review polling tests should prefer real timers because fake timers can stall `findBy...` waits around MSW-backed job polling. #testing #frontend #timers
