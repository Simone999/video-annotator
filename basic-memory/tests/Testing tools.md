...
Use smallest tool set that proves boundary you care about.
Do not drag browser stack into backend contract tests. Do not drag full E2E into screen-level checks.
Use [[Tests Index]] when you first need right test layer. Use this note after boundary is already chosen.
Choose layer from feature notes and product code first; do not use existing tests as router.

## How to use this note

1. Choose boundary first in [[Tests Index]] or owning feature note.
2. Read only tool row for that boundary.
3. Run one exact verification command after writing test.
...
| `pytest-cov` | Report coverage for backend test runs. | Run `pytest --cov=app` or repo coverage command when coverage matters. Repo-level `npm run test` now fails below `90%` backend line or branch coverage. | [ctx7](https://context7.com/pytest-dev/pytest-cov) |
...
| Tool | Purpose | How to use | Guide |
| --- | --- | --- | --- |
| `Vitest` | Run frontend unit and component tests with mocks and snapshots. | Run `npm --workspace frontend run test`; repo and frontend test commands now collect coverage and fail below `90%` frontend line or branch coverage. Use `vi` for mocks, spies, and timers. | [ctx7](https://context7.com/vitest-dev/vitest) |
...
| `@testing-library/react` | Render React UI and query it way user sees it. | Render component, then query by role, label, text, and other user-facing selectors. | [ctx7](https://context7.com/testing-library/react-testing-library) |
...
| `@testing-library/jest-dom` | Add expressive DOM matchers such as `toBeInTheDocument()`. | Load matcher setup once in `frontend/tests/setup/vitest.setup.ts`, then use richer DOM assertions in component tests. | [ctx7](https://context7.com/testing-library/jest-dom) |
...
| `dev-browser` | Run manual browser smoke and incremental browser automation on local stack. | Use for one-off visual checks, screenshot evidence, and small repeated browser workflows; keep scripts small and follow [[Using dev-browser for browser smoke verification]]. | [[Using dev-browser for browser smoke verification]] |
| `MSW` | Mock API calls at request layer across tests and local UI work. | Reuse shared server in `frontend/tests/setup/msw/server.ts`; register handlers per test or per story as needed. | [ctx7](https://context7.com/mswjs/msw) |
...
## Observations
- [tooling] Unit tests in this repo usually use `pytest` or `Vitest`, depending on whether local rule lives in backend or frontend code. #unit #testing
...
- [coverage] Repo-level `npm run test` now enforces `90%` line and branch coverage on both backend and frontend; frontend Vitest coverage includes `src/**/*.{ts,tsx}` and excludes tests, Storybook stories, and declaration files only. #testing #coverage #pytest-cov #vitest
