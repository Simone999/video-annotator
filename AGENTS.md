...
- real SAM2 propagation runtime should reuse same `Sam2Service` lazy predictor state as prompt work; map `direction = "both"` onto forward/reverse `propagate_in_video(...)` calls that mirror `_resolve_target_frame_indices`, and let job persistence filter out seeded start frame instead of trimming it inside adapter code
